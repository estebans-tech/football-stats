...
<ul class="space-y-2">
  ...
  <li class="grid grid-cols-1 sm:grid-cols-[1fr_auto] items-center gap-3 py-3 pl-3 border-b border-white/10">
    <!-- Left: name/nickname or inputs -->
    <div class="min-w-0">
      {#if !editing}
        {#if player.nickname}
          <div class="font-medium truncate text-white">{player.nickname}</div>
        {/if}
        <div class="text-sm truncate text-gray-300">{player.name}</div>
      {:else}
        ...
        <div class="mt-2 flex flex-wrap justify-end gap-2">
          ...
          <button type="button" class="btn btn-utility" onclick={() => (editing = false)}>
            {$t('common.cancel')}
          </button>
        </div>
      {/if}
    ...
    <div class="flex flex-wrap justify-end gap-2">
      ...
      <button
        type="button"
        class={`btn ${isArchived() ? 'btn-warning' : 'btn-utility'}`}
