...
export const pushLineups = async () => {
  const db = assertDb();
  const last = await getLastSync('sync.lineups.push');
  ...
  for (const r of changed) {
    ...
    const row = {
      ...
      half: r.half ?? 1,
      // behåll din domän ('A'/'B') eller mappa till 'home'/'away'
      team: r.team ?? 'A',
    ...
    if (!prev || row.updated_at > prev.updated_at) byKey.set(k, row);
  }
  const rows = [...byKey.values()];

  // 2) försäkra oss om att alla refererade matcher finns i molnet
  ...
  if (!filtered.length) { await setLastSync('sync.lineups.push', Date.now()); return { pushed: 0 }; }

  // 3) upsert på unika nyckeln (match_id,player_id,half)
  const { error } = await sb.from('lineups')
    .upsert(filtered, { onConflict: 'match_id,player_id,half' });
  if (error) throw error;

  await setLastSync('sync.lineups.push', Date.now());
